...
import { Search, Plus, User, Building2, Eye, Pencil, Trash2 } from 'lucide-react';
import { getAccessToken } from '@/src/lib/auth';
import { apiFetch } from '@/src/lib/api';
import type { Client, ClientsResponse, ClientCreateRequest, ClientCreateResponse, ClientUpdateRequest } from '@/src/lib/types/client';
...
export default function ClientesPage() {
  ...
  const handleSubmitClient = async (data: ClientCreateRequest | ClientUpdateRequest) => {
    try {
      setFormLoading(true);
      setError(null);
      ...
      if (editingClient) {
        ...
        const response = await apiFetch<ClientCreateResponse>(`/clients/${clientId}`, {
          method: 'PATCH',
      ...
      if (err instanceof Error) {
        ...
        const apiErr = err as { message?: string; status?: number; details?: unknown };
        ...
        if (apiErr.details) {
          ...
          if (typeof apiErr.details === 'object') {
            const details = apiErr.details as { message?: string; error?: string; [key: string]: unknown };
            ...
            // Si hay un array de errores (validación)
              ...
              const errorMessages = details.errors
                .filter((e: unknown) => typeof e === 'string' || (typeof e === 'object' && e !== null && 'message' in e))
                .map((e: unknown) => {
        ...
        if (errorMessage === 'Error desconocido. Por favor, intenta nuevamente.' && apiErr.message) {
      ...
      if (errorMessage === 'Error desconocido. Por favor, intenta nuevamente.') {
  ...
  const getClientType = (client: Client): 'Empresa' | 'Persona Física' => {
    return client.legalName && client.legalName.trim() !== '' ? 'Empresa' : 'Persona Física';
  };
      ...
      <main className="ml-60 pt-16 p-6">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="relative flex-1 w-full sm:max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar por nombre, placas, RFC, email..."
              value={searchTerm}
        ...
        <div className="mb-6 flex gap-2 border-b border-gray-200 dark:border-zinc-800">
          <button className="px-4 py-2 border-b-2 border-blue-600 text-blue-600 dark:text-blue-400 font-semibold flex items-center gap-2">
            <User size={18} />
            Clientes
          </button>
          <button className="px-4 py-2 text-gray-500 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-300 flex items-center gap-2">
            <Building2 size={18} />
            Vehículos
          </button>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-zinc-800">
                  {filteredClients.map((client) => {
                    ...
                    return (
                      <tr key={client.id || client._id} className="hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            {clientType === 'Empresa' ? (
                              <Building2 size={20} className="text-gray-400 dark:text-zinc-400" />
                            ) : (
                              <User size={20} className="text-gray-400 dark:text-zinc-400" />
                            )}
                            <div>
        ...
        {/* Modal de Vista (TODO: implementar) */}
        {viewingClient && (
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Nombre</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Email</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.email}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Teléfono</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.phone}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">RFC</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.taxId}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Razón Social</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.legalName || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Dirección</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.address}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Ciudad</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.city}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Estado</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.state}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Código Postal</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.zipcode}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Contacto Preferido</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.preferredContact}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Email de Facturación</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1">{viewingClient.billingEmail}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-500 dark:text-zinc-400">Estado</label>
