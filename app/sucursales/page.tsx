...
import type { Branch, BranchesResponse, BranchCreateRequest, BranchCreateResponse, BranchUpdateRequest } from '@/src/lib/types/branch';
...
export default function SucursalesPage() {
    ...
    try {
      ...
      if (response.ok && Array.isArray(response.data)) {
        ...
        if (response.data.length > 0) {
          ...
          console.log('Todas las keys del primer branch:', Object.keys(response.data[0]));
        }
        
        // Normalizar los branches para asegurar que tengan id
        const normalizedBranches = response.data.map((branch: Branch & { _id?: string; ID?: string }) => ({
          ...branch,
  ...
  const handleSubmitBranch = async (data: BranchCreateRequest | BranchUpdateRequest) => {
    try {
      setFormLoading(true);
      setError(null);
      ...
      if (editingBranch) {
        ...
        console.log('Actualizando sucursal - editingBranch completo:', editingBranch);
        console.log('ID de la sucursal:', editingBranch.id);
        console.log('Datos a enviar:', data);
        
        const response = await apiFetch<BranchCreateResponse>(`/branches/${editingBranch.id}`, {
          method: 'PATCH',
        ...
        if (response && response.ok && response.data) {
          ...
          const message = response?.message || 'Error al actualizar la sucursal';
      ...
      if (err instanceof Error) {
        ...
        if (apiErr.message) {
          ...
          const details = apiErr.details as { message?: string; [key: string]: unknown };
  ...
  const getAccentColor = (branch: Branch, index: number): 'blue' | 'purple' | 'green' => {
    ...
    try {
      if (branch.metadata) {
        ...
        if (metadata.color) {
          ...
          if (colorHex.startsWith('#2563eb') || colorHex.startsWith('#3b82f6') || colorHex.includes('blue')) {
            return 'blue';
          }
          if (colorHex.startsWith('#9333ea') || colorHex.startsWith('#a855f7') || colorHex.includes('purple')) {
            return 'purple';
          }
          if (colorHex.startsWith('#22c55e') || colorHex.startsWith('#10b981') || colorHex.includes('green')) {
            return 'green';
          }
        }
      }
    ...
    const colors: Array<'blue' | 'purple' | 'green'> = ['blue', 'purple', 'green'];
